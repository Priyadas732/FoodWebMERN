...
const SignUp = () => {
  const [showToast, setShowToast] = React.useState({visible: false, message:'', icon: null});      
  ...
  const handleSubmit = async (e) => {
      ...
      try {
        ...
        if(res.data.success && res.data.token){
          ...
          setShowToast({visible: true, message:'Sign Up successfull!', icon: <FaCheckCircle />});
          return; 
        }
             ...
             {showToast && <AwesomeToast message ="Sign Up Successful" icon={<FaCheckCircle />}/>}
               ...
               border-4 border-amber-700/30 transform transition-all duration-300 hover:shadow-2xl'>
                 ...
                 <form onSubmit={handleSubmit} className=' space-y-4'>
                          ...
                          className=' w-full px-4 py-3 bg-[#2D1B0E] text-amber-100 placeholder-amber-400 focus:outline-none
                           ...
                           <button className=' absolute inset-y-0 right-4 flex items-center text-amber-400 hover:text-amber-600 transition-all
                            ...
                            <button type='submit' className=' w-full py-3 bg-gradient-to-r from-amber-400 to-amber-600 text-[#2D1B0E]
                              font-bold rounded-lg hover:scale-105 transition-transform duration-300
                              hover:shadow-lg '>
                                Sign Up
                              </button>
                 </form>
