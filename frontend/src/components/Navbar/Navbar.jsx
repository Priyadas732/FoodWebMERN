...
import { useCart } from '../../CartContext/CartContext'; // Placeholder for cart context
const Navbar = () => {
  const [isOpen, setIsOpen] = useState(false);
  const navigate = useNavigate();
  const location = useLocation();
  const {totalItems} = useCart(); // Placeholder for cart items count
  ...
  const renderMobileAuthButton = () => {
     return isAuthenticated ? (
      <button onClick={handleLogout} className=' w-full px-4 py-3 bg-gradient-to-br from-amder-500 to-amber-700
      ...
      <button onClick={() => { 
         navigate('/login')
         setIsOpen(false)
       }} className=' w-full px-4 py-3 bg-gradient-to-br from-amder-500 to-amber-700
      text-[#2D1B0E] rounded-xl font-semibold flex items-center justify-center space-x-2 text-sm '>
        <FiKey/>
        <span>Login</span>
  ...
  return (
    <nav
      className='bg-[#2D1B0E] border-b-8 border-amber-900/30 shadow-[0_25px_50px_-12px_rgba(69,26,3,0.3)] 
            sticky top-0 z-50 font-vibes group/nav overflow-x-hidden'
    >
      ...
      <div className='max-w-7xl mx-auto px-4 relative'>
        <div className=' flex justify-between items-center h-26 md:h-20 lg:h-24'>
          {/* LOGO SECTION */}
        ...
        ${
          isActive
            ? 'border-amber-600/50 bg-amber-900/20 shadow-[0_0_15px] shadow-amber-500/20'
            : 'border-amber-900/30 hover:border-amber-600/50 '
        } shadow-md shadow-amber-900/20`
                  }>
                  <span className=' text-amber-100'>{link.icon}</span>
              ...
              md:ml-3 lg:ml-6 mr-2 md:mr-3 lg:mr-4'>
                ...
                relative border-2 border-amber-900/30 hover:border-amber-600/50 hover:bg-amber-900/20 '>
        ...
        shadow-amber-900/30 w-full'>
          <div className=' px-4 py-4 space-y-2'>
            ...
            <div className=' pt-4 border-t-2 border-amber-900/30 space-y-2'>
               ...
               className='w-full px-4 py-3 text-center text-amber-100 rounded-x1 border-2 border-amber-900/30 hover:border-amber-600/50 flex 
