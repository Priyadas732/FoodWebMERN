...
const MyOrder = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get('https://foodie-pee-backend.onrender.com/api/orders', {
    ...
    <div className=' min-h-screen bg-gradient-to-br from-[#1a120b] via-[#2a1e14] to-[#3e2b1d] py-12 px-4 sm:px-6 lg:px-8'>
       <div className=' mx-auto max-w-7xl'>
             ...
             <div className='bg-[#4b3b3b]/80 backdrop-blur-sm rounded-3xl p-8 shadow-2xl border-2 border-amber-500/20'>
                          ...
                          <tbody>
                                {
                                  orders.map((order) => {
                                     ...
                                     const totalPrice = order.total ?? order.items.reduce(
                                       ...
                                       return (
                                            <tr key={order._id} className=' border-amber-500/20 hover:bg-[#3a2b2b]/30
                                                    ...
                                                    <div className=' space-y-2'>
                                                        {order.items.map((item, index) => (
                                                             ...
                                                             className=' flex items-center gap-3 p-2 bg-[#3a2b2b]/50 rounded-lg'>
                                                                 <img src={`https://foodie-pee-backend.onrender.com${item.item.imageUrl}`} alt={item.item.name}
                 ...
                 {orders.length === 0 && (
                          <div className=' text-center py-12 rext-amber-100/60 text-xl'>
                                       No Orders Found
                          </div>
                 )}
             </div>
