        ...
        <div className={layoutClasses.card}>
          ...
          <div className={layoutClasses.wrapper}>
            <table className={layoutClasses.table}>
              <thead className={layoutClasses.headerRow}>
                <tr>
              ...
              <tbody>
                {orders.map(order => {
                  ...
                  const payMethod = paymentMethodDetails[order.paymentMethod] || { class: 'bg-gray-300 text-gray-800', label: 'Unknown' };
                    ...
                    <tr key={order._id} className={tableClasses.row}>
                        ...
                        <div className='flex items-center gap-2'>
                          ...
                          <div>
                            <p className='text-amber-100'>
                              {order.user?.name || `${order.firstName || ''} ${order.lastName || ''}`.trim() || 'Guest User'}
                            </p>
                            <p className='text-sm text-amber-400/60'>{order.phone}</p>
                            <p className='text-sm text-amber-400/60'>{order.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className={tableClasses.cellBase}>
                      ...
                      <td className={tableClasses.cellBase}>
                        <div className='flex flex-col gap-2'>
                          <div className={`${payMethod.class} px-3 py-1.5 rounded-lh border text-sm`}>
                            {payMethod.label}
                          </div>
                          <div className={`${payStatusStyle.class} flex items-center gap-2 text-sm`}>
