...
const List = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try{
         const {data} = await axios.get('https://foodie-pee-backend.onrender.com/api/items');
         setItems(data);
      }
      catch(err){
  ...
  const handleDelete = async (itemId) => {
    ...
    try{
      await axios.delete(`https://foodie-pee-backend.onrender.com/api/items/${itemId}`);
  ...
  const renderStars = (rating) => 
     [...Array(5)].map((_, i) => (
      <FiStar className={`text-xl ${i < rating ? 'text-amber-400 fill-current' : 'text-amber-100/30'}`} key={i}/>
     ))
     if(loading){
      return (
                  ...
                  {items.map(item => (
                    <tr key={item._id} className={styles.tr}>
                      <td className={styles.imgCell}>
                        <img src={item.imageUrl || undefined} alt={item.name} className={styles.img} />
                      </td>
